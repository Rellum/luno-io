...
LunoBook.prototype.state = function (state) {
  if (state) {
    ...
    if (state.asks && state.bids) {
      this._state = state
      this._orders = []
      this._state.asks.forEach(function (ask) {
        ask.type = 'ASK'
        lunoBook._orders.push(LunoOrder(ask))
      })
      this._state.bids.forEach(function (bid) {
        bid.type = 'BID'
      ...
      return true
    } else if ((parseInt(this._sequence) + 1).toString() === state.sequence) {
      ...
      if (state.trade_updates) {
        state.trade_updates.forEach((tradeUpdate) => {
          ...
          if (order.processTradeUpdate(tradeUpdate) === false) {
...
LunoBook.prototype.getMaxBidPrice = function () {
...
LunoBook.prototype.getMinAskPrice = function () {
...
LunoBook.prototype.getTicker = function () {
  if (this._orders) {
    let lastTrade = null
    if (this._trades[0]) {
    ...
    return {
      ask: this.getMinAskPrice(),
      timestamp: this._timestamp,
      bid: this.getMaxBidPrice(),
