...
LunoBook.prototype.getOrderAsync = function (orderId) {
  const lunoBook = this
  return new Promise(function promise (resolve, reject) {
    let order = lunoBook.getOrder(orderId)
    if (order) {
      resolve(order)
    } else {
      lunoBook.eventEmitter.on('marketChange', function onMarketChange () {
        order = lunoBook.getOrder(orderId)
        if (order) {
          lunoBook.eventEmitter.removeListener('marketChange', onMarketChange)
          resolve(order)
        }
      })
    }
    setTimeout(function timeout () {
      reject(new Error('took too long to resolve' + orderId))
    }, 10000)
  })
}
...
LunoBook.prototype.getTicker = function (options) {
  if (this.isLive()) {
    const ticker = {
      ask: this.getBestPrice(Object.assign({fromCurrency: 'XBT', toCurrency: 'ZAR'}, options)),
      timestamp: this._timestamp,
...
LunoBook.prototype.isOrderMarketable = function (order) {
  if (order.getType() === 'ASK') {
    ...
    const isDeepEnough = (typeof depth === 'undefined' || typeof order.getVolume() === 'undefined' || depth >= order.getVolume())
    return (order.getPrice() >= this.getBestPrice({fromCurrency: 'XBT', toCurrency: 'ZAR'}) && isDeepEnough)
  }
}
